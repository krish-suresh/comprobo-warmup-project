...
class PersonFollowerNode(Node):
    ...
    def run_loop(self):
        ...
        if self.dist_to_person is None:
            vels = Twist(linear=Vector3(x=SPEED, y=0.0, z=0.0), angular=Vector3(x=0.0,y=0.0,z=0.0))
        else:
            rotation_speed = self.angle_to_person*Kp_theta
            linear_speed = (self.dist_to_person-PERSON_DIST)*Kp_dist
            vels = Twist(linear=Vector3(x=linear_speed, y=0.0, z=0.0), angular=Vector3(x=0.0,y=0.0,z=rotation_speed))
